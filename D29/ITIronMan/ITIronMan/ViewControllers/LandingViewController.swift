...
class LandingViewController: UIViewController {
    ...
    @IBAction func pushMajorInvestorDidTap(_ sender: Any) {
        
        let storyboard = UIStoryboard(name: "MajorInvestors", bundle: nil)
        if let vc = storyboard.instantiateViewController(withIdentifier: "MajorInvestorsViewController") as? MajorInvestorsViewController {
            navigationController?.pushViewController(vc, animated: true)
        }
    }
}
